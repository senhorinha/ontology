		...
		try {
			...
			for (OWLClass owlClass : owlClasses) {
				...
				if (owlClass.toString().contains(classeEscolhida)) {
					...
					List<OWLNamedIndividual> animaisCandidatos = new ArrayList<OWLNamedIndividual>(simpleInference.getIndividuals(owlClass));
					...
					for (OWLClassExpression eClass : eClasses) {
						...
						while (!possibleAnswers.contains(respostaUsuario)) {
							...
							System.out.println("Seu animal " + part1.toLowerCase() + " " + part2.toLowerCase() + "? (s/n/naosei/talvez)"); // reposta
